...
setwd("~/Documents/phd/code/trees")
...
mycols <- adjustcolor(palette(rainbow(8)), alpha.f = 0.3)


palette(c(rgb(188,145,129, maxColorValue=255),
          rgb(1,146,39, maxColorValue=255),
          rgb(232,0,64, maxColorValue=255),
          rgb(2,171,166, maxColorValue=255),
          rgb(90,37,0, maxColorValue=255),
          rgb(177,211,80, maxColorValue=255),
          rgb(66,23,39, maxColorValue=255),
          rgb(255,156,140, maxColorValue=255)))
palette()
maxy <- max(AllData[1:8,])
miny <- min(AllData[1:8,])
...
plot(1:2047, a[1,], type = "l", col = mycols[1], ylim = c(-10000, 10000))
for(i in 1:8) {
  lines(1:2047, a[i+1,], col = mycols[i+1])
}
...
plot(483:1191, b[1,483:1191], type = "l", col = mycols[1], ylim = c(-10000, 10000))
for(i in 1:8) {
  lines(483:1191, b[i+1,483:1191], col = mycols[i+1])
}
plot(1192:2047, b[1,1192:2047], type = "l", col = mycols[1], ylim = c(-10000, 10000))
for(i in 1:8) {
...
g <- pcys$`7` 
h <- pcys$`8` 
nn <- max(length(a), length(b), length(c), length(d), length(e), length(f), length(g), length(h))
...
lmo <- rbind(a,b,c,d,e,g,f,h)
z <- cbind(lmo,catagories)
z
z<-as.data.frame(z)
names(z) <- c('PC1', 'X', 'I', 'D', 'block')
aov <- aov(PC1 ~ block + X*I*D, data = z)
aov
...
m <- pcys$`1`* pcys$`2` * pcys$`3` * pcys$`4` * pcys$`5` * pcys$`6` * pcys$`7` * pcys$`8`
